...
describe TopTeams do
  let(:top_teams) { TopTeams.new }
  ...
  context "when processing first game of matchday 1" do
    subject { top_teams.process(matchday1_game1_result) }

    it "records score but outputs nothing" do
      expect(top_teams).to_not receive(:print_results)
      subject
      expect(top_teams.scores).to eq({
  ...
  context "when processing all games of matchday 1" do
    subject do
      top_teams.process(matchday1_game1_result)
      top_teams.process(matchday1_game2_result)
      top_teams.process(matchday1_game3_result)
    end
  ...
  context "when processing first game of matchday 2" do
    ...
    subject { top_teams.process(matchday2_game1_result) }
  ...
  context "when invalid game result is passed" do
    ...
    it "does nothing" do
      top_teams.process(invalid_game_result)
      expect(top_teams.scores).to eq({})
    end
  end
end
